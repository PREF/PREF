...
DisassemblerWorker::DisassemblerWorker(QHexEditData *hexeditdata, ProcessorLoader *loader, bool elaborateinstructions, bool analyzelisting, LogWidget *logwidget, QLabel *infolabel, QObject *parent): QThread(parent), _elaborateinstructions(elaborateinstructions), _analyzelisting(analyzelisting), _listing(nullptr), _hexeditdata(hexeditdata), _loader(loader), _logwidget(logwidget), _infolabel(infolabel)
{
...
void DisassemblerWorker::run()
{
    ...
    this->_loader->disassemble(this->_infolabel, this->_elaborateinstructions, this->_analyzelisting);
